package net.kloudspace.trades.registry;
...
import net.kloudspace.trades.entity.NewVillager;
...
public class Registry {
	 ...
	 private static final Registry INSTANCE = new Registry();
	    ...
	    public interface IVillageTradeHandler
	    {
	         ...
	         * @param recipeList
	         */
	        void manipulateTradesForVillager(NewVillager villager, MerchantRecipeList recipeList, Random random);
	    }
	     ...
	     * @param random
	     */
	    public static void manageVillagerTrades(MerchantRecipeList recipeList, NewVillager villager, int villagerType, Random random)
	    {
	    ...
	    @SuppressWarnings("unchecked")
	    public static void addEmeraldBuyRecipe(NewVillager villager, MerchantRecipeList list, Random random, Item item, float chance, int min, int max)
	    {
	        if (min > 0 && max > 0)
	        {
	        	NewVillager.villagersSellingList.put(item, new Tuple(min, max));
	        }
	        NewVillager.func_146091_a(list, item, random, chance);
	    }

	    @SuppressWarnings("unchecked")
	    public static void addEmeraldSellRecipe(NewVillager villager, MerchantRecipeList list, Random random, Item item, float chance, int min, int max)
	    {
	        if (min > 0 && max > 0)
	        {
	            NewVillager.blacksmithSellingList.put(item, new Tuple(min, max));
	        }
	        NewVillager.func_146089_b(list, item, random, chance);
	    }

	    public static void applyRandomTrade(NewVillager villager, Random rand)
	    {
