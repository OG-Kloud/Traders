...
public class KloudTools implements IVillageTradeHandler{
    ...
    public void manipulateTradesForVillager (EntityVillager villager, MerchantRecipeList recipeList, Random random) {
		...
		case 9:
			getStack();
			Reference.logger.info("Ingredent 1: " +ingredient);
			getStack2();
			Reference.logger.info("Ingredient 2: " +ingredient2);
			getCost(ingredient);
			Reference.logger.info("Cost: " +cost);
			getCost2(ingredient2);
			Reference.logger.info("Cost 2: " +cost2);
			stackCost(ingredient, ingredient2,cost,cost2);
			Reference.logger.info("Stack Cost: " +total);
			getResultStack(total);
			Reference.logger.info("Result Stack: " +result);
			if(ingredient.stackSize < 10 && ingredient2 != null) {
				recipeList.addToListWithCheck(new MerchantRecipe(modified,modified2,result));
				Reference.logger.info("Added Double");
			}
			else {
				recipeList.addToListWithCheck(new MerchantRecipe(modified, result));
				Reference.logger.info("Added Single");
			}
		case 14:
			getStack();
			getCost(ingredient);
